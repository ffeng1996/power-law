...
def main(prune_fraction, model='mlp', num_epochs=500):
	# Load the dataset
	print("Loading data...")
...
############################### thresholding the weights ##########################
	...
	for i in range(len(param_values)):
		...
		if len(param_values[i].shape)>1 :   ########### this time, all the layers are sparse
			vec_data = data_current.flatten()
			a = int(prune_fraction*data_current.size)
			thres_current = np.sort(vec_data)[a]
		else:
			thres_current = np.float32(0.0)  ### all the b are retrained
...
############################### retraining ########################################
	...
	for epoch in range(num_epochs):
		...
		if val_acc > best_val_acc:
			...
			np.savez('model/sparse_model_{0}_{1}_{2}.npz'.format(prune_fraction, depth, width), *lasagne.layers.get_all_param_values(network_s))
