...
def plot(degree, option, layer, adjust_axis, xaxis_labels):
    ...
    plt.tight_layout()
    p.savefig('figures/powerlaw_clauset/mnist_{0}_{1}.png'.format(option, layer), bbox_inches='tight')
...
def main(filename, option, adjust_axis):
    ...
    net['conv1'] = ConvLayer(net['input'], 32, 5)
    net['pool1'] = PoolLayer(net['conv1'], 2)
    net['conv2'] = ConvLayer(net['pool1'], 32, 5)
    net['pool2'] = PoolLayer(net['conv2'], 2)
    net['fc3'] = DenseLayer(net['pool2'], num_units=256)
    net['fc4'] = DenseLayer(net['fc3'], num_units=10, nonlinearity=None)
    net['prob'] = NonlinearityLayer(net['fc4'], softmax)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filename", type=str, dest="filename",
                        default='model/sparse_cnn_0.7.npz')
